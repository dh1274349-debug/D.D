...
<head>
  ...
  <title>Diario Personal — Deportista</title>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* Contenedor principal con más espacio entre columnas */
    ...
    .form-section{
        ...
        gap: 12px; /* Espacio entre los campos de la sección */
    }
    .form-section h3{
    ...
    input[type="text"], input[type="date"], textarea, select{
      width:100%;padding:10px;border-radius:10px;border:1px solid var(--section-border); /* Borde más visible */
    ...
    <div>
      <div class="card">
        <div class="header">
          ...
          <div>
            <h1>Diario Personal — Deportista</h1>
            <div class="small">Registra el día a día y recibe sugerencias de técnicas.</div>
          </div>
        </div>
        ...
        <form id="diaryForm" class="space" onsubmit="return false;">
          ...
          <div class="form-section">
            ...
            <div style="display:flex;gap:12px;align-items:center;">
              ...
              <label style="width:220px">
                <div class="small">Enfermedad / condición</div>
          ...
          <div class="form-section">
            <h3>🧘 Emociones y Sentimientos</h3>
            <div class="grid-2">
              <label>
              ...
              <label>
                <div class="small">Sentimientos (más detalle)</div>
          ...
          <div class="form-section">
            <h3>🚨 Seguimiento de Posible Crisis</h3>
            <label>
              <div class="small">Posible crisis del día</div>
              <textarea id="posible_crisis" placeholder="Describe en pocas líneas"></textarea>
            </label>
            ...
            <div class="grid-2">
              ...
              <label>
                <div class="small">Duración de la crisis</div>
          ...
          <div class="form-section">
            <h3>🎯 Técnica Recomendada y Feedback</h3>
            <label>
              <div class="small">Técnica que se recomienda (puedes añadir manualmente o elegir una sugerencia)</div>
              <textarea id="tecnica_recomendada" placeholder="Aquí se añadirán técnicas recomendadas"></textarea>
            </label>

            <div class="grid-2">
              <label>
                <div class="small">Experiencia con la técnica</div>
            ...
            <div class="grid-2">
              ...
              <label>
                <div class="small">Mejora y retroalimentación</div>
          ...
          <div class="actions">
            ...
            <div style="margin-left:auto;display:flex;gap:8px">
              <button class="btn ghost" id="suggestBtn" type="button">Sugerir técnicas (IA)</button>
          ...
          <div class="space"></div>
          <div class="small notice">Sugerencias IA: si tienes un endpoint seguro llamado <code>/api/ai-suggest</code> (o similar), la página intentará usarlo. Si no, se usan sugerencias locales heurísticas (privadas, en tu navegador).</div>
        </form>
      </div>
    ...
    <aside>
      <div class="card">
        <div style="display:flex;justify-content:space-between;align-items:center">
          <div>
            <h3 style="margin:0">Sugerencias IA / Técnicas</h3>
            <div class="small muted">Añade una sugerencia a la técnica recomendada.</div>
          </div>
        ...
        <div>
          <strong class="small muted">Consejos rápidos</strong>
          <ul class="small muted" style="margin-top:8px">
            <li>Guarda entradas diariamente para ver mejoras.</li>
            <li>Usa la exportación para backup.</li>
            <li>Si integras OpenAI, no guardes claves en el navegador en producción.</li>
          </ul>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="kpi muted">
            ...
            <div class="tag" id="lastSaved">Última: —</div>
          </div>
 <script>
    // ------------------------
    // Lógica JavaScript (todo en un archivo)
    // ------------------------
    (function(){
      // Elementos
      ...
      function onSave(){
        ...
        if (!data.objetivos && !data.emociones && !data.posible_crisis && !data.enfermedad){
          if (!confirm('Parece que la entrada está casi vacía. ¿Deseas guardarla?')) return;
        }
        if (editingId){
        ...
        renderEntries();
        onClearForm();
        notify('Entrada guardada', 'Recuerda revisar tus técnicas y anotaciones.');
      }
      ...
      function onImport(e){
        ...
        reader.onload = function(ev){
          try{
            ...
            save();
            renderEntries();
            notify('Importación completa','Entradas importadas correctamente.');
          } catch(err){
      ...
      function onSuggest(){
        ...
        fetch(endpoint, {
          ...
          // Si hubo error (no server), usar heurística local
          const heur = localSuggestions(payload.emociones, payload.sentimientos, payload.posible_crisis);
          renderAISuggestions(heur, false);
      ...
      function renderAISuggestions(list, fromServer=false){
        ...
        list.forEach((s, i) => {
          ...
          el.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:flex-start">
                              <div style="font-size:14px"><strong>Técnica ${i+1}</strong></div>
                            ...
                            <div style="margin-top:8px;display:flex;gap:8px">
                              <button class="btn ghost" data-idx="${i}">Añadir al campo</button>
                            </div>`;
        ...
        aiContainer.querySelectorAll('button').forEach(btn=>{
          btn.addEventListener('click', (ev)=>{
            ...
            notify('Añadido', 'Sugerencia añadida a "Técnica recomendada".');
          });
        });
      }
      ...
      function renderEntries(){
        ...
        entries.forEach(e=>{
          ...
          el.innerHTML = `<div style="display:flex;justify-content:space-between;align-items:flex-start">
                              <div>
                                ...
                                <div class="muted" style="margin-top:6px;font-size:13px">${escapeHtml(e.fecha)} — ${truncate(e.objetivos,60)}</div>
                              </div>
        ...
        lastSaved.textContent = entries.length ? ('Última: ' + entries[0].fecha) : 'Última: —';
      }
      ...
      // ------------------------
      // Heurística local para sugerencias (si no hay servidor)
      // ------------------------
      function localSuggestions(emoc, senti, crisis){
        ...
        if (/(ansios?|ansiedad|nervios|tenso|estres)/i.test(text)){
          suggestions.push("Respiración diafragmática — Pasos: 1) Sentado con espalda recta. 2) Inhala 4s por la nariz. 3) Retén 2s. 4) Exhala 6s por la boca. Repetir 5 ciclos. Uso: antes o durante la sesión para bajar activación.");
          suggestions.push("Relajación muscular progresiva — Pasos: 1) Tensa un grupo muscular 5s. 2) Suelta y observa. 3) Recorre todo el cuerpo. Duración: 10-12 min. Uso: tras entrenamiento o cuando note tensión.");
        }
        if (/(triste|deprim|desanim|sin ganas|agotad)/i.test(text)){
          suggestions.push("Activación conductual corta — Pasos: 1) Realiza 1 tarea pequeña y concreta (5-10 min). 2) Registra cómo te sentiste. 3) Recompénsate. Duración: 10-20 min. Uso: cuando haya baja motivación.");
          suggestions.push("Registro de gratitud dirigida — Pasos: 1) Escribe 3 cosas que hiciste bien. 2) Relaciónalas con el entrenamiento. 3) Léelas en voz alta. Duración: 5 min. Uso: diaria, tras sesiones.");
        }
        if (/(ira|frustr|molest)/i.test(text)){
          suggestions.push("Técnica STOP — Pasos: 1) Para la acción. 2) Toma 3 respiraciones largas. 3) Observa pensamientos y actúa con intención. Duración: 2-5 min. Uso: en picos de irritación.");
        }
        if (/(panico|ataque|insufrible)/i.test(text)){
          suggestions.push("Enraizamiento 5-4-3-2-1 — Pasos: 1) Nombra 5 cosas que ves. 2) 4 sonidos. 3) 3 sensaciones. 2 olores. 1 cosa que puedas saborear. Duración: 3-5 min. Uso: crisis aguda.");
        }
        // Sugerencia general de movilidad
        suggestions.push("Micro-pausa activa — Pasos: 1) 2 min de movilidad suave. 2) 30-60s de respiración consciente. 3) Reanuda con objetivo reducido. Uso: cuando la sesión se siente abrumadora.");

        // Si no detecta nada concreto
        if (suggestions.length === 0){
          return [
            "Respiración 4-4-4 — Inhala 4s, retén 4s, exhala 4s — 4 ciclos — Uso: antes de competir o si nota bloqueo.",
            "Activación breve — Haz 3 series de 30s de trabajo ligero con intención de enfoque (ej. saltos suaves) — 5-10 min — Uso: para mejorar ánimo antes de entrenar."
          ];
        }
        return suggestions;
      }
